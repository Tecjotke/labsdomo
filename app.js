// ══════════════════════════════════════════════════════════════════════════════
// CONFIGURACIÓN DE SUPABASE - ¡COLOCA TUS CREDENCIALES AQUÍ!
// ══════════════════════════════════════════════════════════════════════════════
const SUPABASE_URL = 'https://mplmmyeurhknpobtswmv.supabase.co';      // Ej: 'https://xxxxx.supabase.co'
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im1wbG1teWV1cmhrbnBvYnRzd212Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUwNTA3MDMsImV4cCI6MjA5MDYyNjcwM30.kQhAz8Sedc6DhrDsfiKWArtiolLBB0rRLvj7zxj2RkA'; // Tu anon/public key
...
// 
// 4. En el SQL Editor de Supabase, ejecuta este script para crear la tabla:
//
...
function initSupabase() {
  if (SUPABASE_URL === 'TU_SUPABASE_URL' || SUPABASE_KEY === 'TU_SUPABASE_ANON_KEY') {
    console.warn('⚠️ Supabase no está configurado. Los datos se guardarán solo localmente.');
  ...
  try {
    ...
    // Usar la librería de Supabase desde CDN
...
function updateConnectionStatus(isOnline) {
  ...
  if (statusEl) {
    if (isOnline) {
      ...
      statusEl.innerHTML = '<span class="dot"></span> Base de datos conectada';
    } else {
      statusEl.className = 'connection-status offline';
      statusEl.innerHTML = '<span class="dot"></span> Modo offline';
    }
  }
}
...
async function saveParticipantToDatabase(name, email, timeSeconds, timeFormatted) {
  if (!isSupabaseConfigured || !supabaseClient) {
    console.log('📝 Guardando localmente (Supabase no configurado)');
    return null;
  }
  ...
  try {
    ...
    console.log('✅ Participante guardado en la base de datos:', data);
    return data;
  } catch (error) {
...
async function loadLeaderboardFromDatabase() {
  ...
  try {
    ...
    leaderboard = data.map(row => ({
      ...
      timeStr: row.time_formatted,
      completedAt: row.completed_at
    }));
    
    console.log(`✅ Leaderboard cargado: ${leaderboard.length} participantes`);
    return leaderboard;
  } catch (error) {
    console.error('❌ Error de conexión:', error);
    return leaderboard;
...
function subscribeToLeaderboard() {
  ...
  try {
    supabaseClient
      .channel('lab_participants_changes')
      .on('postgres_changes', 
        { event: 'INSERT', schema: 'public', table: 'lab_participants' },
        (payload) => {
          console.log('🔔 Nuevo participante detectado:', payload.new);
          // Agregar al leaderboard local y re-renderizar
          const newEntry = {
            name: payload.new.name,
            email: payload.new.email,
            time: payload.new.time_seconds,
            timeStr: payload.new.time_formatted,
            completedAt: payload.new.completed_at
          };
          
          // Solo agregar si no existe ya
          const exists = leaderboard.some(e => 
            e.email === newEntry.email && e.time === newEntry.time
          );
...
async function doLogin() {
  ...
  // Mostrar loading
  btn.disabled = true;
  btn.innerHTML = '<span class="loading-indicator"></span> Iniciando...';

  participant = { name, email };
  
  // Cargar leaderboard desde la base de datos
  ...
  // Restaurar botón (por si se usa después)
  btn.disabled = false;
...
async function triggerFinish() {
  ...
  await saveParticipantToDatabase(participant.name, participant.email, elapsed, timeStr);
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  // Cargar leaderboard inicial si Supabase está configurado
